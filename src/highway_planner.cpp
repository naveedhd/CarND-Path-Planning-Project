...
HighwayPlanner::HighwayPlanner() : lane_(1), ref_vel_(0.0) {
...
void HighwayPlanner::operator()(uWS::WebSocket<uWS::SERVER> ws, char *data,
  ...
  if (car_ahead) {  // Car ahead
    if (!car_left && lane_ > 0) {
      ...
      lane_--;  // Change lane left.
    } else if (!car_righ && lane_ != 2) {
      // if there is no car right and there is a right lane.
      lane_++;  // Change lane right.
    } else {
      speed_diff -= MAX_ACC;
    }
  } else {
    if (lane_ != 1) {  // if we are not on the center lane.
      if ((lane_ == 0 && !car_righ) || (lane_ == 2 && !car_left)) {
        lane_ = 1;  // Back to center.
      }
    }
    if (ref_vel_ < MAX_SPEED) {
      speed_diff += MAX_ACC;
    }
  }
  ...
  vector<double> next_wp0 = getXY(car_s + 30, 2 + 4 * lane_, map_waypoints_s_,
                                  map_waypoints_x_, map_waypoints_y_);
  vector<double> next_wp1 = getXY(car_s + 60, 2 + 4 * lane_, map_waypoints_s_,
                                  map_waypoints_x_, map_waypoints_y_);
  vector<double> next_wp2 = getXY(car_s + 90, 2 + 4 * lane_, map_waypoints_s_,
                                  map_waypoints_x_, map_waypoints_y_);
